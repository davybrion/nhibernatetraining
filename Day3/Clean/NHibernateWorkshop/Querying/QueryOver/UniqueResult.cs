...
namespace NHibernateWorkshop.Querying.QueryOver
{
    ...
    public class UniqueResult : AutoRollbackFixture
    {
        ...
        [Test]
        public void singleordefault_returns_one_value_instead_of_list_with_one_value()
        {
        ...
        [Test]
        public void singleordefault_fails_when_resultset_contains_more_than_one_row()
        {
            Session.Save(new CustomerBuilder().WithName(_customerName).Build());
            Session.Save(new CustomerBuilder().WithName(_customerName).Build());
            FlushAndClear();
